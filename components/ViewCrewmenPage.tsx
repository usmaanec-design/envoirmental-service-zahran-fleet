...
interface ViewCrewmenPageProps {
  lang: Language;
  crewmen: Labour[];
  onUpdateLabour: (labour: Labour) => Promise<void>;
  onDeleteLabour: (labourId: string) => Promise<void>;
  onDeleteSelectedCrewmen: (labourIds: string[]) => Promise<void>;
  onImportCrewmen: (data: any[]) => Promise<{ success: boolean, error?: string }>;
  isReadOnly?: boolean;
}

const ViewCrewmenPage: React.FC<ViewCrewmenPageProps> = ({ lang, crewmen, onUpdateLabour, onDeleteLabour, onDeleteSelectedCrewmen, onImportCrewmen, isReadOnly = false }) => {
    ...
    const filteredCrewmen = useMemo(() => {
        if (!searchTerm.trim()) return crewmen;
        const lowercasedFilter = searchTerm.toLowerCase();
        return crewmen.filter(c =>
    ...
    const handleConfirmDelete = async () => {
        ...
        if (labourToDelete) {
            try {
                ...
                const message = e instanceof Error ? e.message : "Failed to delete crewman.";
    ...
    const handleConfirmBulkDelete = async () => {
        ...
        if (selectedIds.length > 0) {
            try {
                await onDeleteSelectedCrewmen(selectedIds);
                setSelectedIds([]);
                setIsBulkDeleteModalOpen(false);
            } catch (e) {
                const message = e instanceof Error ? e.message : "Failed to delete selected crewmen.";
                setActionError(message);
            }
        }
    };
    ...
    const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            try {
                ...
                const result = await onImportCrewmen(data);
                 if (result.success) {
    ...
    const exportData = useMemo(() => {
        ...
        if (filteredCrewmen.length === 0) {
            return [headers];
        }

        return filteredCrewmen.map(c => ({
            [t.labourName]: c.name,
    ...
    return (
        <div className="w-full bg-white dark:bg-gray-800 rounded-xl shadow-sm hover:shadow-md border border-gray-100 dark:border-gray-700 p-4 sm:p-6 transition-all duration-200">
             <div className="flex flex-wrap justify-between items-center mb-5 pb-4 border-b border-gray-100 dark:border-gray-700 gap-4">
                <h2 className="text-2xl sm:text-3xl font-bold text-orange-600 dark:text-orange-400">{t.viewCrewmen}</h2>
                        ...
                        {selectedIds.length > 0 && !isReadOnly && (
                         ...
                         {!isReadOnly && (
                            <>
                                ...
                                <button onClick={handleImportClick} className="bg-purple-600 hover:bg-purple-700 text-white px-3.5 py-2 rounded-lg text-xs sm:text-sm font-semibold flex items-center gap-1.5 shadow-sm transition-all hover:shadow cursor-pointer" title={t.importCrewmen}>
                                    <i className="fas fa-file-import text-xs sm:text-sm"></i>
                                    <span>{t.import}</span>
                                </button>
                            </>
                         )}
                        <ExportButtons data={exportData} title={t.viewCrewmen} />
                     </div>
                </div>
            </div>
            ...
            {filteredCrewmen.length > 0 ? (
                <Table<Labour>
                    columns={columns}
                    data={filteredCrewmen}
                    initialSortKey="name"
                ...
                <div className="text-center py-16 text-gray-500 dark:text-gray-400">
                    ...
                    <p className="text-lg">{t.noCrewmenFound}</p>
                </div>
            )}
...
export default ViewCrewmenPage;
